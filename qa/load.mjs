...
async function сценарийE() {
  ...
  const out = []
  // Потолки подняты (см. api/set-exercise.js): маячок 120 в минуту, воронка
  // 300. Шлём чуть больше потолка — проверяем, что он там, где объявлен, а не
  // «где-то выше».
  out.push(await проба('/api/boot (потолок 120/мин)', `${APP}/api/boot`, 'нагрузочная проверка', 130))
  out.push(await проба('воронка (потолок 300/мин)', `${APP}/api/set-exercise?action=funnel`, { event: 'нет-такого-события' }, 310))
  /**
   * А ЭТО ГЛАВНОЕ: ветка с токеном. Полсотни запросов с одного адреса без
   * токена должны упираться не в 30 старого лимита, а в заслон 300 — то есть
   * пройти все и получить честный 401, а не 429.
   */
  const безТокена = await залп(50, () => замер(`${APP}/api/set-exercise?action=motion-log`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}',
  }))
  out.push({
    ветка: 'set-exercise без токена, 50 с одного адреса',
    послано: 50,
    отказов: безТокена.записи.filter((r) => r.status === 429).length,
    коды: безТокена.записи.reduce((a, r) => ({ ...a, [r.status]: (a[r.status] ?? 0) + 1 }), {}),
  })
