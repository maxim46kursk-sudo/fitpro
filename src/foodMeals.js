...
// Приём из МАРКЕРА АССИСТЕНТА → ключ для базы, либо null.
//
// Белый список, а не «что пришло, то и пишем»: значение приходит из ответа
// модели, то есть снаружи. Чужая строка не должна доехать до insert — там
// CHECK-ограничение food_diary.meal, и запись просто отвалится с ошибкой,
// а человек увидит «не удалось сохранить» вместо еды в дневнике.
//
// Неизвестное или отсутствующее значение → null. Это не потеря: NULL — та
// самая служебная секция «Без категории», в которую и раньше попадало всё.
// Честнее показать запись без приёма, чем выбросить её или угадать приём за
// человека.
export const normalizeMeal = value => {
  if (typeof value !== 'string') return null
  const key = value.trim().toLowerCase()
  return MEAL_KEYS.includes(key) ? key : null
}
