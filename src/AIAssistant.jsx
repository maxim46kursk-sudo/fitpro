...
const AIAssistant = forwardRef(function AIAssistant({ workoutHistory = [], isMobile = false, nutritionPlans = [], userId = null }, ref) {
  ...
  // Записи, добавленные в дневник в течение этой сессии чата — держим в памяти,
  // чтобы промпт сразу видел то что AI только что записал в Supabase, даже если
  // экран дневника сейчас не открыт и не успел перечитать данные из базы.
  ...
  const flashDiaryToast = () => {
    setShowDiaryToast(true)
    setTimeout(() => setShowDiaryToast(false), 2000)
  }
  ...
  const buildSystemPrompt = (mode) => {
    ...
    // 4. Съедено сегодня — база localStorage (зеркало Supabase, обновляется экраном дневника)
    // + записи этой сессии чата, на случай если экран дневника ещё не перечитал Supabase
    const diary = (() => {
      const stored = (() => { try { return JSON.parse(localStorage.getItem('fitpro_food_diary') || '{}') } catch { return {} } })()
      const merged = { ...stored }
      Object.entries(sessionDiaryEntries).forEach(([d, entries]) => {
        const existingIds = new Set((merged[d] || []).map(e => String(e.id)))
        const fresh = entries.filter(e => !existingIds.has(String(e.id)))
        if (fresh.length) merged[d] = [...(merged[d] || []), ...fresh]
      })
      return merged
    })()
...
Когда записываешь полный рацион — используй отдельный маркер DIARY_ENTRY для каждого приёма пищи. Каждый маркер на отдельной строке. В поле name обязательно указывай название приёма — Завтрак, Перекус, Обед, Ужин. Пример: [DIARY_ENTRY:{"name":"Завтрак — овсянка и яйца","kcal":420,"p":20,"c":45,"f":17}]
...
Отвечай на русском. Без markdown, без звёздочек, без решёток. Чистый текст.`
  ...
  const send = async () => {
    ...
    try {
      ...
      if (data.content?.[0]?.text) {
        ...
        // 1. Запись в дневник питания — может быть несколько приёмов пищи за раз,
        // каждый в своём маркере. Пишем ТОЛЬКО в Supabase (единственный источник правды —
        // экран дневника читает из food_diary, а не из localStorage).
        const diaryMatches = [...rawText.matchAll(/\[DIARY_ENTRY:(\{[^}]+\})\]/g)]
        if (diaryMatches.length) {
          const { data: { user } } = await supabase.auth.getUser()
          if (user) {
            const date = new Date().toISOString().slice(0, 10)
            const newEntries = []
            for (const dm of diaryMatches) {
              try {
                const entry = JSON.parse(dm[1])
                ...
                else newEntries.push({ id: data.id, name: data.name, kcal: data.kcal, p: data.p, c: data.c, f: data.f })
              } catch (e) { console.error('Ошибка разбора DIARY_ENTRY:', e) }
            }
            if (newEntries.length) {
              setSessionDiaryEntries(prev => ({ ...prev, [date]: [...(prev[date] || []), ...newEntries] }))
              window.dispatchEvent(new CustomEvent('fitpro:diary-update'))
              diaryWritten = true
              flashDiaryToast()
            }
          }
          rawText = rawText.replace(/\[DIARY_ENTRY:[^\]]+\]/g, '')
        }
        ...
        if (goalsMatch) {
          ...
          rawText = rawText.replace(/\[SET_GOALS:[^\]]+\]/g, '')
        }

        // 3. Удалить конкретную запись по id — только Supabase
        ...
        if (deleteMatch) {
          try {
            ...
            const { data: { user } } = await supabase.auth.getUser()
            if (user) {
              const { error } = await supabase.from('food_diary').delete().eq('id', id).eq('user_id', user.id)
              if (error) console.error('Ошибка удаления записи:', error)
              window.dispatchEvent(new CustomEvent('fitpro:diary-update'))
            }
            actionDone = 'entry_deleted'
          } catch {}
          rawText = rawText.replace(/\[DELETE_ENTRY:[^\]]+\]/g, '')
        }

        // 4. Очистить весь дневник за дату — только Supabase
        ...
        if (clearMatch) {
          try {
            ...
            const { data: { user } } = await supabase.auth.getUser()
            if (user) {
              const { error } = await supabase.from('food_diary').delete().eq('user_id', user.id).eq('date', date)
              if (error) console.error('Ошибка очистки дневника:', error)
              window.dispatchEvent(new CustomEvent('fitpro:diary-update'))
            }
            actionDone = `cleared_${date}`
          } catch {}
          rawText = rawText.replace(/\[CLEAR_DIARY:[^\]]+\]/g, '')
        }

        // Убираем пустые строки, оставшиеся после вырезания маркеров — ответ должен быть компактным
  ...
  const savePlanDayToDiary = async (msgId, date) => {
    ...
    const { data, error } = await supabase.from('food_diary').insert(entries.map(e => ({
      user_id: user.id, date, name: e.name,
      kcal: +e.kcal||0, p: +e.p||0, c: +e.c||0, f: +e.f||0,
    }))).select()
    if (error) { console.error('Ошибка записи рациона в дневник:', error); return }

    setSessionDiaryEntries(prev => ({
      ...prev,
      [date]: [...(prev[date] || []), ...data.map(r => ({ id: r.id, name: r.name, kcal: r.kcal, p: r.p, c: r.c, f: r.f }))],
    }))
    window.dispatchEvent(new CustomEvent('fitpro:diary-update'))
    flashDiaryToast()
    ...
    <>
      ...
      {isOpen && (
        <div style={{
          ...
          {/* Тост об успешной записи в дневник */}
          {showDiaryToast && (
            <div style={{
              position: 'absolute', top: 14, left: '50%', transform: 'translateX(-50%)',
              zIndex: 1200, display: 'flex', alignItems: 'center', gap: 7,
              padding: '10px 18px', borderRadius: 24, background: '#16a34a', color: '#fff',
              fontSize: 13, fontWeight: 700, boxShadow: '0 6px 20px rgba(22,163,74,0.35)',
            }}>
              Записано в дневник ✓
            </div>
          )}
