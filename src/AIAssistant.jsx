...
${matchedPlan
  ...
  : `   Готового рациона под этот вес нет — составь рацион сам, ориентируясь на норму (${normKcal ?? '?'} ккал, Б:${normP ?? '?'}г У:${normC ?? '?'}г Ж:${normF ?? '?'}г). Используй тот же формат что и в готовых рационах: 5 приёмов пищи (Завтрак, Перекус, Обед, Перекус, Ужин) с указанием времени, конкретных продуктов с граммовкой, калорий и макросов каждого приёма, и итого за день. Используй простые продукты (гречка, куриная грудка, рыба, яйца, творог, овощи, фрукты). После рациона напиши: "Это индивидуальный рацион, составленный специально под твои данные."`}
