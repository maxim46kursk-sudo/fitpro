...
// Иконка и описание программы живут в базе (program_templates.icon/description):
// программ будет много, каждая добавляется данными, без правки кода и выкладки.
// Эти два словаря — ЗАПАСНОЙ вариант для первых четырёх зашитых программ, если
// в их строке базы колонки пустые (или базы нет вовсе). Стоят здесь, а не рядом
// с остальными константами ниже: mergeTemplates читает их и вызывается на
// загрузке модуля (TemplatesContext), до инициализации нижней части файла.
      ...
      {infoFolder&&createPortal(
        <div onClick={()=>setInfoFolder(null)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.4)', zIndex:1300, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:SURF, borderRadius:16, padding:'22px 20px', maxWidth:340, width:'100%', boxSizing:'border-box' }}>
            ...
            <div style={{ fontSize:13, color:TXT3, textAlign:'center', lineHeight:1.5, marginBottom:18 }}>{folderDescription(infoFolder)}</div>
    ...
    <div style={{ position:'fixed', inset:0, background:SURF2, zIndex:2100, display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'14px 16px 24px' }}>
        {loading?(
          ...
          <div style={{ marginBottom:12 }}>
            <div style={{ fontSize:11, color:TXT3, marginBottom:6 }}>Иконка программы</div>
            <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
              {PROGRAM_ICONS.map(n=>(
                <button key={n} onClick={()=>setIcon(n)} title={n}
                  style={{ width:44, height:44, borderRadius:12, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', padding:0, minHeight:'unset',
                    border:`1.5px solid ${icon===n?PUR:HAIR}`, background:icon===n?'#EEEDFE':'transparent' }}>
                  <GlassIcon name={n} size={26} />
                </button>
              ))}
            </div>
          </div>
          {/* Описание — текст карточки-инфо («?» в списке программ). */}
          <div style={{ marginBottom:12 }}>
            <div style={{ fontSize:11, color:TXT3, marginBottom:4 }}>Описание программы</div>
            <textarea value={description} onChange={e=>setDescription(e.target.value)} maxLength={300} rows={3}
              placeholder="Одна-две фразы: для кого программа и на что упор"
              style={{ width:'100%', padding:'9px 12px', fontSize:13, borderRadius:9, border:`1.5px solid ${HAIR}`, boxSizing:'border-box', outline:'none', color:TXT, resize:'vertical', fontFamily:'inherit' }}
              onFocus={e=>e.target.style.borderColor=PUR} onBlur={e=>e.target.style.borderColor=HAIR} />
  ...
  return (
    <CatalogContext.Provider value={catalogValue}>
     <TemplatesContext.Provider value={templatesValue}>
      ...
      {/**
        * Форма входа рисуется НИЖЕ раздела: пока она ОТКРЫТА, раздела быть не
        * должно, иначе он заберёт себе все нажатия (см. onCreate у OfferSheet).
        *
        * Условие — authOpen, а не showAuth. `showAuth` после входа не
        * сбрасывается: форму гасит `&& !user` в самом authOpen. Проверяй мы
        * `showAuth`, раздел Motion был бы намертво закрыт до конца сессии у
        * всякого, кто вошёл через предложение аккаунта.
        */}
      {motionOpen&&!authOpen&&(
        <MotionOverlay
          onExit={closeMotion}
      ...
      */}
      ...
      {/**
        * ПРИШЕДШЕМУ ПО ПРЯМОМУ АДРЕСУ ПРИВЕТСТВИЕ НЕ ПОКАЗЫВАЕМ. Он пришёл
        * читать про поток, а не знакомиться с приложением: плашка «Пробуй всё»
        * ложится ровно на цену и кнопку «Участвовать» — то есть закрывает
        * собой то, за чем человек и пришёл. Закроет страницу крестиком —
      ...
      {offer&&(
        <OfferSheet
          ...
          onCreate={()=>{
            ...
            setOffer(null)
            /**
             * РАЗДЕЛ ЗАКРЫВАЕМ, ИНАЧЕ ФОРМА ОКАЖЕТСЯ ПОД НИМ.
             *
             * Живая поломка, найденная прогоном пути гостя: раздел Motion лежит
             * оверлеем на z-index 2900, форма входа рисуется ниже. Гость,
             * читавший страницу челленджа, нажимал «Создать аккаунт» — и не
             * происходило ровно ничего: форма была на экране, но перекрыта
             * страницей, и клики забирала она.
             *
             * Раньше этого не могло случиться: гость до страницы челленджа не
             * доходил вовсе, ему показывали предложение прямо на карточке.
             * Открыли страницу — открылась и дыра.
             */
            closeMotion()
            setAuthTabWanted('register')
            setShowAuth(true)
