...
describe('отправка переживает закрытие раздела', () => {
  it('ОТКЛЮЧЕНИЕ ХРАНИЛИЩА НА СЕРЕДИНЕ НЕ ОБРЫВАЕТ ОТПРАВКУ', async () => {
    /**
     * Найдено прогоном на боевом адресе: попытки уезжали, а прогресс молча нет.
     * Причина — порядок в очистке эффекта: раздел отключал хранилище, не дождавшись
     * отправки, и вторая её половина обращалась уже в пустоту. Без попыток запись
     * прогресса успевала уйти в том же кадре, поэтому вручную это не ловилось.
     */
    writeJson(KEYS.challenge, { day: 5, done: [{ day: 4, at: 'вчера' }] })
    writeJson(KEYS.challengeAttempts, день(4, 'pro', [попытка(900, '2026-08-01T10:00:00Z')]))

    const saveProgress = vi.fn(async () => {})
    const отключить = configureSync({
      load: async () => null,
      // хранилище отключают ровно в тот момент, когда попытки уже ушли
      saveAttempts: async () => { отключить() },
      saveProgress,
    })
