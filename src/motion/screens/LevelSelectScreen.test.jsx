...
/**
 * ВТОРОЙ КОМНАТЫ У УЧАСТНИКА НЕТ.
 *
 * «Моя комната» внутри игры — со своим календарём тридцати дней, своими очками
 * и показателями — стала дублем комнаты участника. Две комнаты с одинаковыми
 * цифрами в разных местах человек читает как «какая-то из них врёт», и
 * правильного ответа на это нет. У одиночки другой комнаты нет вовсе — ему всё
 * как было, и решает это ХОЗЯИН экрана (index.jsx), передавая onRoom или не
 * передавая: сам экран по-прежнему рисует кнопку ровно тогда, когда её дали.
 */
describe('вторая комната', () => {
  it('кнопки нет, когда её не дали', () => {
    render(<LevelSelectScreen challengeDay={5} challengeMember onRoom={null} />)
    expect(screen.queryByTestId('open-room')).toBeNull()
  })

  it('и есть, когда дали', () => {
    render(<LevelSelectScreen challengeDay={5} onRoom={() => {}} />)
    expect(screen.queryByTestId('open-room')).toBeTruthy()
  })
})
