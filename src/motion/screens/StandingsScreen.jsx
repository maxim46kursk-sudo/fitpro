...
import { MIN_ENTRIES, MIN_MEALS } from '../../challengeNutrition.js'
      ...
      {!loading && started && table.length > 0 && (
        <>
          ...
          {/**
            ...
            * дневнике и решил, что таблица врёт. Она не врала: порог дня не
            * набрался. Правило было, объяснения не было — а экран, который
            * показывает ноль и молчит о причине, читается как поломка. В
            * таблице, по которой делят деньги, это недопустимо.
            *
            * Числа берутся из судьи (challengeNutrition.js), а не пишутся
            * словами: поменяется порог — поменяется и эта строка.
            */}
          <p className="mt-st__note" data-testid="standings-note">
            День по питанию засчитывается от {MIN_ENTRIES} записей минимум в {MIN_MEALS} приёмах
            пищи — иначе ноль, даже если записи есть. Дни до появления твоей нормы тоже
            считаются нулём.
          </p>
        </>
      )}
