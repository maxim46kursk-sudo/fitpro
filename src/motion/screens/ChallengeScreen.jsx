 ...
 */
/**
 * Шаг «как это работает»: заголовок и текст, одной колонкой во всю ширину.
 *
 * НОМЕРОВ БОЛЬШЕ НЕТ. Они стояли отдельной колонкой слева, и колонка эта
 * держала ширину по самой длинной подписи на всю высоту шага — то есть рядом с
 * коротким текстом оставалась пустая чёрная полоса в рост абзаца. Порядок шагов
 * и так виден: их три, они идут сверху вниз, и «01» этому ничего не добавляло.
 *
 * КАРТИНКА НЕОБЯЗАТЕЛЬНА: у первых двух шагов её нет — калибровку и бой
 * показывает ролик на первом экране, причём в движении. У третьего осталась:
 * дневника питания в ролике нет.
 */
function Step({ title, text, image = null, alt, caption }) {
  return (
    <div className="mt-ch__step">
      <h3 className="mt-ch__stepT">{title}</h3>
      <p>{text}</p>
