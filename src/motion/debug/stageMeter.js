 ...
 *   log       — журнал: складывание строк, снимки состояния, отправка.
 *
 ...
 *
 * МЕДИАНА, А НЕ СРЕДНЕЕ. Один провал на разогреве не должен решать за минуту — а
 * провалы и случаются, когда телефон уходит думать над кадром.
 *
...
export function stageReport() {
  ...
  for (const m of [...minutes.keys()].sort((a, b) => a - b)) {
    ...
    for (const c of COUNTS) {
      const v = median(b.counts[c].values)
