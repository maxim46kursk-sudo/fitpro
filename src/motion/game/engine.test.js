...
describe('девять новых движений: зачёт по событию автомата', () => {
  ...
  it('хлопок в окне — препятствие пройдено, и замер уходит в лог', () => {
    ...
    expect(clear).toBeTruthy()
    /**
     * Хлопок судится пересечением порогов, а не выдержкой (см. CROSSED в
     * moves.js): событие приходит на ПЕРВОМ замере, где поза сложилась, — тут
     * это за 200 мс до пролёта. Было наоборот: выдержка в 200 мс сдвигала
     * событие ровно на пролёт, и на редкой съёмке хлопок из-за неё терялся.
     * Окно вокруг пролёта при этом никуда не делось — оно и решает, засчитан
     * ли хлопок этой плите.
     */
    expect(clear.moveAt).toBe(-200)
  ...
  it('на зачёте замеры те же: по ним видно, с каким запасом движение прошло', () => {
    ...
    /**
     * У хлопка выдержки больше нет — он судится пересечением порогов
     * (см. CROSSED в moves.js), и `held` означает у него другое: НАСКОЛЬКО
     * РАЗЪЕХАЛИСЬ признаки по замерам. Ноль — оба сложились в одном замере;
     * шире pairWindowMs — это уже не одно движение, а два подряд.
     */
    expect(clear.held).toBe(0)
    expect(clear.held).toBeLessThanOrEqual(DEFAULT_MOVES.pairWindowMs)
  })
