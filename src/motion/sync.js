...
export async function hydrate(userId) {
  ...
  if (remoteHas) {
    ...
    /**
     * ИЗВЕСТНО ТО, ЧТО ЛЕЖИТ НА СЕРВЕРЕ, — И ТОЛЬКО ОНО.
     *
     * Здесь стояло `attemptRows(merged.attempts)`, то есть в «уже отправленные»
     * записывался результат СЛИЯНИЯ — вместе с попытками, которые лежали только
     * на устройстве и наверх не уезжали ни разу. Дальше push их молча
     * отфильтровывал как повторы, и уехать они не могли уже никогда.
     *
     * ЦЕНА ОШИБКИ. Заход при этом НЕ пропадал: он ехал в payload прогресса
     * (saveProgress шлёт его целиком) и на глаз всё выглядело правильно —
     * комната показывала свою тысячу очков. Но таблица потока считает места по
     * motion_attempts, а туда строка не попадала, и человек навсегда оставался
     * в таблице с нулём. Ровно это и увидел владелец на проде 25.08: в комнате
     * 1000, в таблице 0. Хуже всего, что расхождение не лечилось ни временем,
     * ни повторным заходом — оно было вечным.
     *
     * Комментарий ниже был правдой замысла и неправдой кода: слияние ДЕЙСТВИТЕЛЬНО
     * должно отправлять локальные попытки наверх, и теперь отправляет — потому
     * что в knownAttempts попадает только серверная сторона.
     */
    for (const r of Array.isArray(remote.attempts) ? remote.attempts : []) {
      knownAttempts.add(`${r.day}:${r.tier}:${r.attempt_no}`)
    }
    // то, что пришло с сервера, серверу возвращать незачем — а всё, чего там
    // нет, уедет наверх этой же отправкой
    schedulePush()
  }
