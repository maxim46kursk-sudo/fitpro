...
// Вода: запись честных нулей. Отличить её от «неизвестных макросов» выше по
// самой записи уже нельзя — там нули стоят вместо прочерков, — поэтому право
// дойти до дневника решается ДО этого места (hasUsableMacros на сервере и
// macrosKnown в сканере), и вода это право теперь имеет.
assertEqual('вода уходит в дневник нулями', buildFoodEntry({ name: 'Вода', kcal100: 0, p100: 0, c100: 0, f100: 0 }, 500),
  { name: 'Вода (500 г)', kcal: 0, p: 0, c: 0, f: 0 })
...
{
  // ВОДА ИЗ OFF — НАХОДКА, А НЕ ПРОМАХ. Здесь стояло «все четыре нуля — не
  // продукт, а незаполненная форма». Из-за этого вода, чай и кофе без молока
  // не находились по штрих-коду вовсе: OFF отдаёт по ним честные нули, а мы
  // объявляли их мусором и отвечали found:false.
  const { res, writes } = await call('4607050690492', {
    off: { status: 1, product: { product_name: 'Вода питьевая', brands: 'Святой Источник', nutriments: { 'energy-kcal_100g': 0, proteins_100g: 0, carbohydrates_100g: 0, fat_100g: 0 } } },
  })
  assertEqual('вода из OFF: найдена', res.body?.found, true)
  assertEqual('вода из OFF: отданы нули, а не прочерки', res.body?.product,
    { barcode: '4607050690492', name: 'Вода питьевая', brand: 'Святой Источник', kcal100: 0, p100: 0, c100: 0, f100: 0, source: 'off' })
  assertEqual('вода из OFF: закэширована', writes.length, 1)
  assertEqual('вода из OFF: в кэш ушли нули', writes[0]?.kcal100, 0)
}
{
  // Вода уже в нашем справочнике — отдаём из кэша, в OFF не ходим.
  const WATER_ROW = { barcode: '4607050690492', name: 'Вода питьевая', brand: 'Святой Источник', kcal100: 0, p100: 0, c100: 0, f100: 0, source: 'off' }
  const { res, writes } = await call('4607050690492', { cache: { '4607050690492': WATER_ROW } })
  assertEqual('вода из кэша: найдена', res.body?.found, true)
  assertEqual('вода из кэша: cached=true', res.body?.cached, true)
  assertEqual('вода из кэша: та же карточка', res.body?.product, WATER_ROW)
  assertEqual('вода из кэша: в базу не писали', writes.length, 0)
}
{
  const { res, writes } = await call('3017620422003', {
...
// ── Годность карточки: известны все четыре числа ───────────────────────────
...
// ВОДА. Здесь стоял отказ «все четыре нуля — незаполненная форма». Он был
// неверен: незаполненная форма даёт null, а нули — это явный ответ. Правило
// запрещало воду, чай и кофе без молока целиком: карточка есть, а ручка
// отвечает «не нашёл», сохранить нельзя, в дневник не занести.
assertEqual('вода 0/0/0/0 — ГОДНА', hasUsableMacros(M(0, 0, 0, 0)), true)
assertEqual('ноль калорий при ненулевых макросах — годна', hasUsableMacros(M(0, 0, 0.5, 0)), true)
...
// легли в справочник как ТОЧНЫЕ. Проверка ловит оба.
{
  ...
  // ВОДА. Атвотер от нулей даёт ожидаемые 0 ккал, и делить на них нельзя:
  // наивное (kcal - expected) / expected выдало бы NaN (0/0) и молча уронило
  // сравнения с порогами в false. Отсюда ранний выход по expected <= 0.
  assertEqual('вода 0/0/0/0 — сходится', c(0, 0, 0, 0).ok, true)
  assertEqual('вода: претензии нет вовсе (плашка не всплывёт)', c(0, 0, 0, 0).kind, null)
  assertEqual('вода: ожидаемая калорийность посчитана как 0, а не NaN', c(0, 0, 0, 0).expected, 0)
  report('вода: NaN в ответе нет', !Number.isNaN(c(0, 0, 0, 0).deviation), JSON.stringify(c(0, 0, 0, 0)))
  // Ноль макросов, но заявлены калории — это уже не вода, а ошибка чтения.
  assertEqual('0 макросов при 250 ккал — не принимаем', c(250, 0, 0, 0).kind, 'too_high')
  // ── Абсолютный потолок расхождения: 20 ккал ─────────────────────────────
  //
  // На околонулевых числах процент бессмыслен. Кола зеро: 0 ккал заявлено,
  // 0.3 г углеводов — по Атвотеру 1.2 ккал, то есть −100% и жалоба too_low.
  // Разница в 1.2 ккал меньше округления на этикетке, и плашки быть не должно.
  assertEqual('кола зеро (0 ккал, 0.3 г У): плашки нет', c(0, 0, 0.3, 0).kind, null)
  assertEqual('кола зеро: сходится', c(0, 0, 0.3, 0).ok, true)
  // Чай с ложкой сахара в составе: 2 г углеводов → 8 расчётных ккал.
  assertEqual('напиток 0 ккал при 2 г углеводов — сходится', c(0, 0, 2, 0).kind, null)

  // Потолок не прощает настоящих ошибок. У Pringles расхождение 302 ккал.
  assertEqual('расхождение 302 ккал остаётся too_high', c(749, 9, 33, 31).kind, 'too_high')
  // Ровно 300 ккал мимо — тоже претензия, а не «мелочь».
  assertEqual('расхождение 300 ккал остаётся too_high', c(747, 9, 33, 31).kind, 'too_high')
  // Занижение на 300 ккал ловится с другой стороны.
  assertEqual('занижение на 300 ккал остаётся too_low', c(147, 9, 33, 31).kind, 'too_low')

  // Границы самого порога. 20 ккал — ещё шум, 21 — уже разбираем по процентам.
  // Берём мелкий продукт, где 21 ккал даёт больше 40% и потому ловится.
  assertEqual('расхождение ровно 20 ккал — сходится', c(30, 0, 12.5, 0).kind, null)
  assertEqual('расхождение 21 ккал на мелком продукте — уже претензия', c(29, 0, 12.5, 0).kind, 'too_low')

  // ГЛАВНОЕ, ЧТО ПОРОГ НЕ ДОЛЖЕН БЫЛ СЛОМАТЬ: у молока с переставленными Ж↔У
  // расхождение всего 7.1 ккал (60 против 67.1) — меньше потолка. Поставь
  // абсолютную проверку первой, и перестановка перестала бы находиться вовсе.
  // Поэтому она стоит ПОСЛЕ поиска обмена столбцов.
  assertEqual('молоко с перестановкой ловится, хотя расхождение всего 7 ккал',
    c(60, 3.2, 3, 4.7).kind, 'swapped')

  // А вот ровные нули претензий не вызывают — это и есть случай воды.
  assertEqual('ровные нули претензий не вызывают', c(0, 0, 0, 0).kind, null)
}
...
{
  // ВОДА СОХРАНЯЕТСЯ. Здесь стоял отказ 400 на «все четыре нуля»: карточку
  // воды, чая или кофе без молока завести было нельзя вовсе — сервер отвечал
  // «заполни калорийность, белки, жиры и углеводы» на полностью заполненную
  // форму. Нули — это ответ, а не молчание.
  const WATER = { barcode: '4607050690492', name: 'Вода питьевая', brand: 'Святой Источник', kcal100: 0, p100: 0, c100: 0, f100: 0 }
  const { res, writes } = await callSave(WATER)
  assertEqual('вода 0/0/0/0: статус 200', res.statusCode, 200)
  assertEqual('вода: карточка заведена', [res.body?.ok, res.body?.created], [true, true])
  assertEqual('вода: строка записана', writes.length, 1)
  assertEqual('вода: нули записаны нулями, а не потерялись',
    [writes[0]?.kcal100, writes[0]?.p100, writes[0]?.c100, writes[0]?.f100], [0, 0, 0, 0])
  assertEqual('вода: карточка вернулась человеку', res.body?.product?.kcal100, 0)
}
{
  // Нули, пришедшие строками с формы, — тот же законный ответ.
  const { res, writes } = await callSave({ ...CARD, kcal100: '0', p100: '0', c100: '0', f100: '0' })
  assertEqual('нули строками: карточка принята', res.statusCode, 200)
  assertEqual('нули строками: разобраны в числа', writes[0]?.kcal100, 0)
}
{
  // Продукт без жиров: строки «жиры» на пачке нет, человек подтвердил 0.
  const { res, writes } = await callSave({ ...CARD, name: 'Зефир', kcal100: 326, p100: 0.8, c100: 79, f100: 0 })
  assertEqual('без жиров: карточка принята', res.statusCode, 200)
  assertEqual('без жиров: Ж=0 записан', writes[0]?.f100, 0)
  assertEqual('без жиров: created=true', res.body?.created, true)
}
