...
const WEEK_DIARY = [
  ...
  { id: 210, date: '2026-07-03', name: 'Ужин: треска 200г, овощи гриль', kcal: 280, p: 40, c: 10, f: 8 },
...
mk({
  group: 'Аналитика дневника', name: 'удали ужин за 3 июля', ctx: ctxFull(WEEK_DIARY), user: 'удали ужин за 3 июля',
  expect: 'Сам находит запись трески за 03.07.2026 (id:210) по названию+дате и удаляет, не спрашивая ID',
  extra: (t) => {
    const issues = []
    if (asksUserForId(t)) issues.push('спросил у пользователя ID записи')
    const del = markers(t, 'DEL')
    if (!del.length) issues.push('нет маркера DEL')
    else if (del[0].id !== 210) issues.push(`удалил не ту запись (id:${del[0].id}, ожидался 210 — треска за 03.07)`)
    return issues
  },
})
mk({
