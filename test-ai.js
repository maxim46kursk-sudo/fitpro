...
const refusesOffTopic = (text) => /не могу|только[^.\n]{0,60}питан|не по теме|не моя тема|другая тема|обсужда(ю|ть) только|кроме питания|не (в моей|по моей|моя) (компетенц|специализац|част[ьи]|тема|област[ьи])|не связан\S*[^.\n]{0,15}с питанием|заточен (под|на) питани/i.test(text)
