...
export const USER_TABLES = [
  ...
  // Телеметрия раздела Motion (sql/2026-08-18_motion_log.sql): что происходило
  // с камерой, распознаванием и счётом во время тренировки. Персональных данных
  // не содержит намеренно — метка камеры и её deviceId вырезаются и клиентом, и
  // приёмником, — но user_id есть, значит по правилу этого файла место ему
  // здесь. Внешний ключ на auth.users объявлен NO ACTION, поэтому строка обязана
  // удаляться до auth.admin.deleteUser(): без этой записи удаление аккаунта
  // просто отказало бы.
  { table: 'motion_log',            column: 'user_id' },
  // Попытки челленджа Motion (sql/2026-08-18_motion_progress.sql): результат
  // каждого захода. Отдельной таблицей, а не в общем комке, именно потому, что
  // это будущий предмет спора о деньгах — по ней должно быть можно спросить.
  { table: 'motion_attempts',       column: 'user_id' },
  // Прогресс челленджа Motion: день, сданные дни, личные планки, пороги,
  // рекорд. Одна строка на человека.
  { table: 'motion_progress',       column: 'user_id' },
