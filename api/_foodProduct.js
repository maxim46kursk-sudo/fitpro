...
// Ссылка из ответа модели → домен для показа человеку («ozon.ru»).
//
// Показываем ИМЕННО домен, а не ссылку целиком: человеку на экране
// подтверждения нужно понять, откуда числа («из карточки Ozon» — понятно,
// «из головы модели» — тоже), а не открыть страницу. Заодно это снимает
// вопрос доверия к строке: домен уходит в текст, ссылка — никуда, кликнуть по
// сочинённому моделью URL никто не сможет.
//
// Невалидный URL → null. Модель иногда возвращает «—» или название магазина
// словами; это не ошибка ответа, просто источника не будет в подписи.
export function cleanHost(v) {
  ...
  try {
    ...
    const host = u.hostname.replace(/^www\./, '').slice(0, 80)
    // Разобрать URL мало: new URL('https://—') не падает, а возвращает
    // punycode-«хост» xn--8ug. Пускать такое в подпись «по данным …» нельзя —
    // это не источник, это мусор из ответа модели. Требуем то, что домен и
    // есть: метки из букв, цифр и дефисов, и зона хотя бы из двух букв
    // (кириллические домены к этому моменту уже приведены к xn--…).
...
export function normalizeLabelProduct(barcode, raw) {
  ...
  const basis = ['estimate', 'web'].includes(raw.basis) ? raw.basis : 'label'
  ...
  return {
    ...
    per,
    basis,
    // Домен источника — только для подписи на экране подтверждения, и только
    // когда числа действительно оттуда. При basis='label' поле бессмысленно
    // (числа с упаковки в кадре), а при 'estimate' — обманчиво: модель иногда
    // прикладывает ссылку «похожего» товара к значениям из своих знаний.
    sourceUrl: basis === 'web' ? cleanHost(raw.src_url) : null,
  }
}
