...
export function cleanNetWeight(v) {
  ...
  // «5 x 80 г», «5×80 г», «255 г», «0,5 л», «330 мл»
  const m = s.match(/^(?:(\d{1,3})\s*[x×*]\s*)?(\d{1,4}(?:[.,]\d{1,2})?)\s*(г|гр|грамм|кг|мл|л|литр|шт)\.?$/)
  if (!m) return null
  const [, mult, num, unitRaw] = m
  const unit = { гр: 'г', грамм: 'г', литр: 'л' }[unitRaw] || unitRaw
  if (!WEIGHT_UNITS.includes(unit)) return null
