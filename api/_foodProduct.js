...
export function normalizeLabelProduct(barcode, raw, { allowedHosts = [] } = {}) {
  ...
  // ЦИТАТА ИЗ ТАБЛИЦЫ — пропуск для basis='label', и проверяем его мы, а не
  // доверяем слову модели.
  //
  // На снимке ЛИЦЕВОЙ стороны творожка модель вернула basis='label' и 124 ккал
  // вместо 99: таблицы в кадре не было вовсе, числа выдуманы, но объявлены
  // снятыми с этикетки — то есть точными, вытесняющими всё. Прочитать таблицу и
  // не суметь её процитировать нельзя; выдумать числа, не видя таблицы, — легко.
  // Поэтому нет цитаты (или в ней нет ни одной цифры) → это не чтение таблицы.
  const quote = cleanText(raw.table_quote, 300)
  if (basis === 'label' && !(quote && /\d/.test(quote))) basis = 'estimate'

  const clean = sanitizeMacros(scaled)

  // СХОДИМОСТЬ КАЛОРИЙНОСТИ С МАКРОСАМИ. Считается для любого источника: числа
  // не сходятся одинаково плохо и когда они выдуманы, и когда прочитаны криво.
  const macroCheck = checkMacros(clean)

  // Не сошлось — «точным чтением таблицы» это больше не считается. Понижаем до
  // прикидки: числа остаются (человек сверяет их глазами и правит), но права
  // вытеснять чужую карточку у них теперь нет, и интерфейс скажет, что не так.
