...
// Карточка годна к употреблению? Известны все четыре числа — и всё.
//
...
//
// ЗДЕСЬ БЫЛ ОТКАЗ «ВСЕ ЧЕТЫРЕ НУЛЯ — НЕЗАПОЛНЕННАЯ ФОРМА». Он снят, и вот
// почему он был неверен: незаполненная форма даёт null, а не нули. Нули — это
// то, что человек или этикетка сказали ЯВНО, и таких продуктов полно: вода,
// чай без сахара, кофе без молока, диетическая газировка. Правило запрещало
// их целиком — воду нельзя было ни найти по коду, ни сохранить, ни занести в
// дневник, причём молча: карточка есть, а ручка отвечает «не нашёл».
//
// Нули пропускаем как есть — отличить «ноль калорий» от «никто не заполнил»
// умеет проверка на null выше, и она остаётся единственной.
export function hasUsableMacros(p) {
  const n = v => (typeof v === 'number' && Number.isFinite(v) ? v : null)
  const kcal = n(p?.kcal100), pr = n(p?.p100), c = n(p?.c100), f = n(p?.f100)
  return kcal !== null && pr !== null && c !== null && f !== null
}
...
// АБСОЛЮТНЫЙ ПОТОЛОК РАСХОЖДЕНИЯ. На околонулевых числах процент теряет смысл:
// у колы зеро заявлено 0 ккал при 0.3 г углеводов, расчёт даёт 1.2 ккал — это
// −100% и жалоба too_low, хотя разница в 1.2 ккал не то что не ошибка, а
// меньше округления на этикетке.
//
// 20 ккал — потолок «шума». Он покрывает напитки и следовые количества
// (0.3 г углеводов дают 1.2 ккал, 2 г — 8) и не покрывает ни одной настоящей
// ошибки из живого прогона: у Pringles расхождение было 302 ккал, у батончика
// на волокнах — 68 (и тот проходит по относительному порогу, а не по этому).
const MACRO_ABS_MAX = 20
...
export function checkMacros(src) {
  ...
  // Расхождение мельче шума — сходится, и разбирать проценты дальше незачем.
  //
  // ПОЧЕМУ ЗДЕСЬ, А НЕ ПЕРВЫМ ДЕЙСТВИЕМ. Соблазн поставить абсолютный порог до
  // всего велик, но он убил бы соседнюю проверку: у молока с переставленными
  // Ж↔У расхождение всего 7.1 ккал (60 заявленных против 67.1 расчётных) — это
  // меньше 20, и перестановка, ради которой ветка 'swapped' и написана,
  // перестала бы находиться. Абсолютный порог гасит ШУМ, а перестановка — не
  // шум: там ошибка видна не по величине, а по тому, что обмен столбцов её
  // резко улучшает. Поэтому сначала ищем обмен, и только потом прощаем мелочь.
  if (Math.abs(kcal - expected) <= MACRO_ABS_MAX) {
