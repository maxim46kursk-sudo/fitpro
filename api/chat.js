...
//  • суточная квота бесплатных живёт в Postgres (incr_feature_usage), потому
//    что в памяти инстанса она бы обнулялась на каждом холодном старте;
...
// Признак в теле 429, по которому клиент выбирает формулировку: упёрся в
// суточную бесплатную квоту (значит, надо предлагать ПРОФИТ) или в почасовой
// потолок (значит, надо просто подождать). Без него оба случая выглядели бы
// одинаково, и бесплатному предлагали бы «попробуй через час» — а у него
// раньше завтрашнего дня ничего не изменится.
...
const LABEL_PROMPT = `На фото — упаковка продукта питания. Извлеки данные с этикетки.

Ответь СТРОГО одним JSON-объектом, без markdown-обёртки и без пояснений:
{"name": string, "brand": string|null, "kcal100": number|null, "p100": number|null, "c100": number|null, "f100": number|null, "per": "100g"|"portion"|"unknown", "portion_g": number|null, "readable": true|false}
...
- kcal100/p100/c100/f100 — калорийность (ккал), белки, углеводы, жиры (граммы) из таблицы пищевой ценности. Каждое поле, которого на этикетке нет, — null.
- per — на какой вес приведена таблица: "100g" — на 100 г продукта, "portion" — на порцию/упаковку, "unknown" — не указано.
- portion_g — вес порции в граммах, если per="portion" и вес указан. Иначе null.
- Числа приводи как есть с этикетки, НИЧЕГО не пересчитывай сам.
- readable — false, если таблицу пищевой ценности на фото прочитать нельзя (размыто, не тот ракурс, её не видно). В этом случае остальные поля можно оставить null.`
...
export default async function handler(req, res) {
  ...
  if (isFoodLabel) {
    ...
    if (level >= AI_MIN_LEVEL) {
      // Платный: почасовой потолок. Считаем ПО ПОЛЬЗОВАТЕЛЮ (id из
      // подписанного токена), а не по IP — за одним оператором связи сидят
      // сотни людей, и IP-лимит бил бы по чужим.
      if (!rateLimit(req, res, { name: 'food-label', limit: PAID_LABELS_PER_HOUR, windowMs: 3_600_000, subject: data.user.id })) return
    } else {
      // Бесплатный/базовый: суточная квота в базе (см.
      // sql/2026-08-05_feature_usage.sql). Инкремент атомарный; считаем и
      // отклонённые попытки — иначе подбором можно было бы крутить счётчик
      // бесконечно, ничего не тратя.
      const { data: used, error: usageError } = await supabaseAdmin
        .rpc('incr_feature_usage', { uid: data.user.id, d: today, k: 'food_label' })
      if (usageError) {
        // Fail closed: без учёта квота не держится, а за каждый снимок платим
        // мы. Молча пускать нельзя.
      ...
      if (used > FREE_LABELS_PER_DAY) {
        return res.status(429).json({
          error: `Лимит ${FREE_LABELS_PER_DAY} фото в день исчерпан`,
          reason: REASON_FREE_DAILY,
        })
      }
    }
