...
// max_uses — потолок расхода, и он важнее, чем кажется по прайсу.
//
// Сам поиск стоит $10 за 1000 запросов — копейка за штуку. Дорого не это:
// НАЙДЕННОЕ приезжает в контекст входными токенами. Замер на живом запросе
// (зефир, 2 поиска): 22 794 входных токена, то есть ~$0.07 против ~$0.02 за
// сами поиски. Итого снимок с поиском обходится примерно в $0.095 ≈ 8 ₽ —
// против ~1.5 ₽ у снимка без поиска.
//
// Отсюда два следствия, которые нельзя терять из виду:
//  • при PAID_LABELS_PER_DAY=100 один платный аккаунт может за сутки выбрать до
//    ~760 ₽, если каждый его снимок пойдёт через поиск;
//  • поэтому шаг 2 и запускается только там, где без него никак (basis=
//    'estimate'), и только платным, и только для товара, которого нет в
//    справочнике.
// Расход виден по счётчику USAGE_KEY_SEARCH — если по нему станет ясно, что
// сотня снимков в день реально выбирается через поиск, здесь нужен отдельный
// суточный потолок поисков, а не только потолок снимков.
...
async function handleFoodLabel(req, res, { userId, barcode, image, paid, supabaseAdmin, today }) {
  ...
  // ── ШАГ 2: поиск. Только если есть ЧТО искать и КОМУ.
  //
  ...
  let searches = 0
  if (product.basis === 'estimate' && paid && !(await alreadyKnown(supabaseAdmin, barcode))) {
