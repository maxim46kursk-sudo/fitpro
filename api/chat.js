...
import { parseModelJson, normalizeLabelProduct, isValidBarcode, isSoftSource } from './_foodProduct.js'
...
// max_uses — потолок расхода. Поиск стоит $10 за 1000 запросов, то есть копейку
// за штуку; три попытки на снимок — это сотые доли цента, но без потолка модель
// на неудачном товаре может уйти в десяток запросов и упереться в наши 55
// секунд, а не в деньги.
...
// vision-прокси.
//
// Из запроса подставляется ровно одно значение — штрих-код, и подставлять его
// БЕЗОПАСНО: parseLabelInput пропускает дальше только строку, прошедшую
// isValidBarcode, то есть 8–14 цифр и ничего больше. Никакого текста, а значит
// и никакой инструкции, в промт отсюда не попадёт.
const labelPrompt = ({ barcode, canSearch }) => `На фото — упаковка продукта питания. Это может быть ЛЮБАЯ сторона упаковки: лицевая с названием или обратная с таблицей пищевой ценности.

ПОСЛЕДНЕЕ сообщение в твоём ответе — СТРОГО один JSON-объект, без markdown-обёртки и без пояснений после него:
{"name": string, "brand": string|null, "kcal100": number|null, "p100": number|null, "c100": number|null, "f100": number|null, "per": "100g"|"portion"|"unknown", "portion_g": number|null, "basis": "label"|"web"|"estimate", "src_url": string|null, "readable": true|false}

Действуй по порядку:

1. ЕСЛИ в кадре видна и ЧИТАЕТСЯ таблица пищевой ценности — возьми точные значения из неё и верни basis="label". ${canSearch ? 'Ничего не ищи: этикетка — главный источник, на упаковке уже есть всё нужное.' : 'Этикетка — главный источник.'}
${canSearch ? `
2. ЕСЛИ в кадре только лицевая сторона — таблицы нет, она обрезана, засвечена или цифры не разобрать, — но продукт опознаётся по названию, марке, оформлению, то СНАЧАЛА НАЙДИ КБЖУ ПОИСКОМ (инструмент web_search), и только потом, если не нашлось, бери числа из своих знаний.
   - Запрос строй из марки и названия с упаковки плюс «пищевая ценность на 100 г». Пример: «Шармэль зефир ванильный пищевая ценность на 100 г».
   - Не нашлось с первого раза — уточни запрос штрих-кодом ${barcode}: по нему находится ровно тот товар, а не соседний по линейке.
   - Поиск ограничен карточками магазинов, где КБЖУ берут у производителя. Годится только страница ЭТОГО товара; подборки, статьи и «похожие товары» — нет.
   - ПРОВЕРЬ, ЧТО НАШЁЛ ТОТ САМЫЙ ВАРИАНТ: вкус, жирность, «классический/лёгкий», вес упаковки должны совпасть с тем, что на фото. «Зефир ванильный» и «зефир в шоколаде» — разные товары с разными числами. Не совпало — это не твой продукт, ищи дальше или переходи к шагу 3.
   - Нашёл — верни числа ровно как на найденной странице, basis="web", src_url — ссылка на неё.
` : `
2. ЕСЛИ таблицы в кадре нет или её не разобрать, НО продукт узнаётся по названию, марке, оформлению упаковки — верни название и марку, а КБЖУ дай ТИПИЧНЫЕ для ИМЕННО ЭТОГО продукта из своих знаний, basis="estimate", src_url=null.
`}
3. ${canSearch ? 'ЕСЛИ поиск ничего надёжного не дал' : 'Давая типичные значения'} — дай ТИПИЧНЫЕ значения для ИМЕННО ЭТОГО продукта из своих знаний, basis="estimate", src_url=null.
...
4. readable=false — ТОЛЬКО если не удалось разобрать даже название продукта (кадр размыт, упаковки в кадре нет, снят посторонний предмет).${canSearch ? ' В этом случае не ищи ничего: искать нечего.' : ''}
...
- per — на какой вес приведены числа: "100g" — на 100 г продукта, "portion" — на порцию/упаковку, "unknown" — не указано. При basis="web" и basis="estimate" давай значения на 100 г и per="100g".
- portion_g — вес порции в граммах, если per="portion" и вес указан. Иначе null.
- src_url — только при basis="web": прямая ссылка на страницу товара, с которой взяты числа. При любом другом basis — null. Без реальной ссылки basis="web" не ставь: карточка без источника — это оценка, а не находка.
...
async function handleFoodLabel(req, res, { userId, barcode, image, paid, supabaseAdmin, today }) {
  // Поиск — платная добавка к и без того платному vision-запросу, поэтому
  // включаем его только с ПРОФИТа (пробный период тоже даёт этот уровень,
  // см. TRIAL_LEVEL в _access.js). Бесплатному СТАРТу остаётся прежнее
  // поведение: распознавание работает, числа при нечитаемой таблице —
  // честная прикидка.
  const canSearch = paid && !(await alreadyKnown(supabaseAdmin, barcode))
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: MODEL,
        max_tokens: canSearch ? LABEL_MAX_TOKENS_SEARCH : LABEL_MAX_TOKENS,
        // Инструмент кладём в запрос, только когда поиск разрешён. Не «кладём
        // всегда, а в промте просим не пользоваться»: неиспользованный
        // инструмент всё равно едет в контекст каждым запросом, а запрет в
        // тексте модель вправе истолковать вольно.
        ...(canSearch ? { tools: LABEL_TOOLS } : {}),
        messages: [{
          role: 'user',
          content: [
            { type: 'image', source: { type: 'base64', media_type: 'image/jpeg', data: image } },
            { type: 'text', text: labelPrompt({ barcode, canSearch }) },
          ],
        }],
      }),
    ...
    if (!response.ok) {
      ...
      console.error(`Этикетка ${barcode} (${userId}): Anthropic ответил ${response.status} (поиск ${canSearch ? 'вкл' : 'выкл'}): ${detail.slice(0, 300)}`)
  ...
  // Текст ответа. Модель отвечает блоками, и с появлением поиска их стало
  // несколько: перед каждым запросом она пишет, что собирается искать, между
  // ними — что нашла, и только ПОСЛЕДНИЙ текстовый блок содержит JSON.
  // Первый блок (как брали раньше) теперь — «Проверю данные по этому товару».
  ...
  // Сколько поисков реально ушло — видно только здесь, и это единственный
  // способ заметить, что модель ищет там, где раньше отвечала сразу (то есть
  // тратит деньги и секунды на каждом скане).
  const searches = blocks.filter(b => b?.type === 'server_tool_use' && b?.name === 'web_search').length

  // Оборванный ответ. Ниже он всё равно уйдёт в 'unreadable' (JSON не
  // разберётся), но причина у него другая, чем «плохое фото», и в логе их надо
  // различать: max_tokens означает, что мал потолок, pause_turn — что модель не
  // уложилась в цикл серверных инструментов.
  if (responseData?.stop_reason === 'max_tokens' || responseData?.stop_reason === 'pause_turn') {
    console.error(`Этикетка ${barcode} (${userId}): ответ оборван, stop_reason=${responseData.stop_reason}, поисков=${searches}`)
  }
  ...
  //
  // Счётчик тут именно счётчик, а не квота: сколько поисков можно за день,
  // задано косвенно — потолком снимков (каждый поиск едет на снимке) и
  // max_uses внутри одного запроса. Отдельный лимит появится, если по этой
  // строке станет видно, что расход того требует.
  //
  ...
  //
  // allowedHosts — тот же белый список, что ушёл в allowed_domains поиска.
  // Ссылка станет кликабельной в интерфейсе, поэтому проверяется дважды: там
  // мы ограничиваем, ГДЕ искать, здесь — что модель приписала к числам.
  const product = normalizeLabelProduct(barcode, parsed, { allowedHosts: SEARCH_DOMAINS })
  if (!product) {
    ...
    console.log(`Этикетка ${barcode} (${userId}): не разобрана (readable/name/JSON), поисков=${searches}`)
