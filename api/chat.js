...
// Ответ короткий и строго структурный, но по дороге модель ещё формулирует
// поисковые запросы и коротко комментирует найденное — это тоже выходные
// токены. 1500 с запасом покрывает три поиска и финальный JSON; упрись мы в
// потолок посреди поиска, ответ пришёл бы вообще без JSON.
const LABEL_MAX_TOKENS = 1500

// Поиск в интернете — серверный инструмент Anthropic: запросы выполняет он
// сам, нам не нужны ни своя ручка (лимит 12 функций Vercel Hobby выбран
// целиком), ни парсинг чужих сайтов.
//
// Зачем вообще: у половины российских товаров нет карточки в Open Food Facts,
// а таблицу КБЖУ на упаковке далеко не всегда видно на снимке — мелкий шрифт,
...
const LABEL_TOOLS = [{ type: 'web_search_20260209', name: 'web_search', max_uses: LABEL_SEARCH_MAX_USES }]
...
const labelPrompt = barcode => `На фото — упаковка продукта питания. Это может быть ЛЮБАЯ сторона упаковки: лицевая с названием или обратная с таблицей пищевой ценности.
...
1. ЕСЛИ в кадре видна и ЧИТАЕТСЯ таблица пищевой ценности — возьми точные значения из неё и верни basis="label". В интернете в этом случае НЕ ищи: на упаковке уже есть всё нужное.

2. ЕСЛИ таблицы в кадре нет, она обрезана, засвечена или цифры не разобрать, НО продукт опознаётся по названию, марке, оформлению упаковки — НАЙДИ КБЖУ ПОИСКОМ В ИНТЕРНЕТЕ (инструмент web_search), а не бери их из своих знаний.
   - Ищи по названию и марке с упаковки, добавив штрих-код ${barcode} — по нему находится ровно тот товар. Пример запроса: «Мр. Рикко майонез перепелиное яйцо 67% калорийность БЖУ 100 г».
   - Числам с ЭТОГО конкретного товара доверяй в таком порядке: сайт производителя, карточка товара в интернет-магазине (Ozon, Яндекс Лавка, Самокат, Перекрёсток, Ашан), затем всё остальное. Агрегаторы «калорийность продуктов» и рецептовые сайты не годятся: там усреднённая категория, а не эта упаковка.
   - ПРОВЕРЬ, ЧТО НАШЁЛ ТОТ САМЫЙ ВАРИАНТ: жирность, вкус, «лёгкий/классический», вес упаковки должны совпасть с тем, что на фото. «Майонез 67%» и «майонез лёгкий 30%» — разные товары с разными числами. Не совпало — это не твой продукт, ищи дальше или переходи к шагу 3.
   - Нашёл — верни числа как на найденной странице, basis="web", src_url — ссылка на страницу, откуда взяты числа.

3. ЕСЛИ поиск ничего надёжного не дал — дай ТИПИЧНЫЕ значения для ИМЕННО ЭТОГО продукта из своих знаний, basis="estimate", src_url=null.
...
4. readable=false — ТОЛЬКО если не удалось разобрать даже название продукта (кадр размыт, упаковки в кадре нет, снят посторонний предмет). В этом случае не ищи ничего: искать нечего.
...
- src_url — только при basis="web": прямая ссылка на страницу, с которой взяты числа. При любом другом basis — null.
...
async function handleFoodLabel(req, res, { userId, barcode, image }) {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            { type: 'text', text: labelPrompt(barcode) },
          ],
        }],
      }),
  ...
  console.log(`Этикетка ${barcode} (${userId}): распознано «${product.name}», basis=${product.basis}, per=${product.per}, поисков=${searches}${product.sourceUrl ? `, источник=${product.sourceUrl}` : ''}`)
