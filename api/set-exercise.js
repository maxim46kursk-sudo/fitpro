...
/**
 * Медиана ряда, а не среднее: одна вкладка, забытая на час, сдвигает среднее
 * так, что оно перестаёт описывать кого бы то ни было. Пустой ряд — null:
 * «никто не дошёл» и «дошли за ноль секунд» — разные ответы.
 */
function медиана(ряд) {
  const с = ряд.filter((н) => typeof н === 'number' && Number.isFinite(н)).sort((a, b) => a - b)
  if (!с.length) return null
  const i = с.length >> 1
  const м = с.length % 2 ? с[i] : (с[i - 1] + с[i]) / 2
  return Math.round(м * 10) / 10
}

/**
 * Запомнить, за сколько секунд человек добрался до отметки. Если он приходил
 * не один раз — берётся САМЫЙ БЫСТРЫЙ заход: второй визит начинается уже с
 * прочитанной страницей, и мешать его с первым значило бы завышать время.
 */
function времяОтметки(карта, кто, сек) {
  const число = typeof сек === 'number' && Number.isFinite(сек) ? сек : null
  const было = карта.get(кто)
  // Человека заводим в любом случае — его считают в «людей» даже без времени
  // (старая запись без поля `сек` — это дошедший человек, а не отсутствующий).
  if (было === undefined || было === null) карта.set(кто, число)
...
export async function собратьСводку(db, сейчас = Date.now()) {
  ...
  /**
   * ЧЕМ СЧИТАЛАСЬ МОДЕЛЬ — по последним заходам.
   *
   * Зачем отдельной строкой. Инференс живёт либо в воркере, либо, если тот не
   * поднялся, на главном потоке — и это две разные архитектуры: на главном
   * модель делит ядро с игровым циклом, и к концу сессии отрисовка теряет
   * кадры (замер 25 августа: 1474 кадра в минуту против 3600 у воркера).
   * Какой режим достался человеку, видно только в журнале, а искать это
   * глазами по тысяче строк нельзя.
   *
   * Наружу идут ТРИ вещи: сколько заходов на каком потоке, сколько откатов и
   * последние причины отказа. Ни личности, ни идентификатора сессии — только
   * счёт и текст ошибки, как и во всей остальной сводке.
   */
  const потоки = async (от) => {
    const { data, error } = await db
      .from('motion_log')
      .select('payload')
      .gte('at', от)
    ...
    for (const строка of data ?? []) {
      for (const line of строка?.payload?.lines ?? []) {
        const тег = тегСтроки(line)
        if (тег !== 'model.ready' && тег !== 'worker.fallback') continue
        let данные
        try { данные = JSON.parse(line.slice(line.indexOf('{'))) } catch { /* строка без полезной нагрузки */ }
        if (тег === 'model.ready') {
          const поток = данные?.thread === 'main' ? 'главный' : 'воркер'
          поПотокам[поток] = (поПотокам[поток] ?? 0) + 1
          if (данные?.shim) сЗаглушкой += 1
        } else {
          откатов += 1
          // Причина — обрезанная: она длинная и повторяется слово в слово.
          if (причины.length < 3) причины.push(String(данные?.why ?? '').slice(0, 160))
        }
      }
    }
    return { поднялось: поПотокам, с_заглушкой_document: сЗаглушкой, откатов, причины }
  }

  /**
   * ВОРОНКА ЛЕНДИНГА ЧЕЛЛЕНДЖА — где сыплется на чтении и где на игре.
   *
   ...
   * ступени мала всегда.
   *
   * ГЛУБИНА — ОТДЕЛЬНОЙ СТРОКОЙ, А НЕ СТУПЕНЯМИ ВОРОНКИ. Между «открыл» и
   * «долистал до цены» лежит вся страница, и отметки 25/50/75 разрезают её на
   * четверти. В ряд ступеней они не встают: это не отдельные шаги к покупке, а
   * разрез одного шага, и вклинив их между `open` и `scroll`, мы сломали бы
   * «процент от предыдущей» у всего, что ниже. Последняя строка разреза —
   * «конец»: это и есть ступень `scroll`, повторять её отдельной отметкой было
   * бы лишней записью в журнал.
   *
   * Рядом с числом людей — МЕДИАННОЕ ВРЕМЯ до отметки. Без него «до половины
   * дошла половина» ничего не решает: три секунды до половины означают, что
   * страницу пролистали большим пальцем, а полторы минуты — что её прочитали, и
   * чинить в этих двух случаях надо противоположное.
   */
  /**
   * ДВЕ ДОРОЖКИ, А НЕ ОДНА. Страница перестроена: на первом экране цены больше
   * нет, там кнопка «Играть». Значит между «открыл» и «долистал до цены» лежит
   * ВТОРАЯ дорожка — человек сначала играет, — и теряют на ней не на тексте, а
   * на камере, калибровке и первом круге. Прежними шестью ступенями это не
   * видно вовсе: они все про чтение.
   *
   * Порядок ровно тот, в котором человек их проходит: открыл -> нажал «Играть»
   * -> камера поднялась -> прошёл калибровку -> начался круг -> круг доигран
   * -> ответил на «Как тебе игра?» -> и дальше обычная продажа.
   */
  const ВОРОНКА = [
    'open',
    'play', 'camera', 'calibrated', 'round-start', 'round-end',
    'up', 'down', 'save',
    'scroll', 'join-click', 'auth', 'pay-start', 'paid',
  ]
  /**
   * ОТ ЧЕГО СЧИТАТЬ ПРОЦЕНТ — СКАЗАНО ЯВНО, А НЕ «ОТ СТРОКИ ВЫШЕ».
   *
   * Дорожек теперь две, и они расходятся сразу после `open`: одни идут играть,
   * другие читают страницу. Строка выше в списке перестала быть предыдущим
   * шагом человека, и «процент от предыдущей» начал сравнивать разное:
   * `scroll` вышел бы долей от `save` (хвоста игры), а `join-click` — долей от
   * `round-end`, хотя до кнопки доходят и те, кто не играл вовсе.
   *
   * `up` и `down` — два ответа на ОДИН вопрос: человек выбирает что-то одно, и
   * оба считаются от `round-end`, а не друг от друга. `save` идёт только за
   * `down`.
   *
   * Ошибка тут дорогая и незаметная: в сводке появился бы «отвал 70%» там, где
   * никакого отвала нет, — а сводку читают, чтобы решать, что чинить.
   */
  const ОТ_ЧЕГО = {
    play: 'open',
    camera: 'play',
    calibrated: 'camera',
    'round-start': 'calibrated',
  ...
  const воронкаЛендинга = async (от) => {
    ...
    const отложены = []
    /** отметка глубины (25/50/75 и 'конец') → посетитель → секунды до неё */
    const доГлубины = new Map([...ГЛУБИНЫ, 'конец'].map((п) => [п, new Map()]))
    ...
    for (const строка of data ?? []) {
      for (const line of строка?.payload?.lines ?? []) {
        ...
        // Глубина ступенью воронки не является: у неё своя строка. Соответствие
        // uid → vid ей не нужно — эти события пишет браузер, vid в них есть
        // всегда, а без него отметку и посчитать не по чему.
        if (шаг === 'depth') {
          const порог = Number(д?.гл)
          if (vid && доГлубины.has(порог)) времяОтметки(доГлубины.get(порог), vid, д?.сек)
          continue
        }
        if (!люди.has(шаг)) continue
        отложены.push({ шаг, vid, uid, сек: д?.сек })
      }
    }
    for (const { шаг, vid, uid, сек } of отложены) {
      const кто = vid || (uid ? uidVid.get(uid) || uid : null)
      if (!кто) continue
      люди.get(шаг).add(кто)
      // «Конец страницы» — это и есть «долистал до цены», отдельного события нет.
      if (шаг === 'scroll') времяОтметки(доГлубины.get('конец'), кто, сек)
    }

    // Ступени с долей от ТОГО шага, за которым эта ступень идёт на самом деле.
    const ступени = []
    for (const шаг of ВОРОНКА) {
      const сколько = люди.get(шаг).size
      const база = ОТ_ЧЕГО[шаг]
      const было = база ? люди.get(база)?.size ?? 0 : 0
      ступени.push({
        ступень: шаг,
        людей: сколько,
        // от чего доля — в самой строке: иначе её читают как «от строки выше»
        от: база ?? null,
        доля: база && было > 0 ? `${Math.round((сколько / было) * 100)}%` : null,
      })
    }
    ...
    // Разрез страницы: сколько людей увидело её четверть, половину, три
    // четверти и конец — и за сколько секунд от открытия.
    const глубина = [...ГЛУБИНЫ, 'конец'].map((п) => {
      const карта = доГлубины.get(п)
      return {
        до: п === 'конец' ? 'конец' : `${п}%`,
        людей: карта.size,
        медиана_сек: медиана([...карта.values()]),
      }
    })

    return { ступени, глубина, по_источникам: поИсточникам, строк_прочитано: data?.length ?? 0 }
  }
...
async function handleGuestChallengeLog(req, res) {
  ...
  // из одной сети, у каждого до семи отметок (четыре ступени плюс три глубины),
  // и запас.
...
async function handleEvents(req, res) {
  ...
  try {
    const db = createClient(SUPABASE_URL, serviceRoleKey)
    // ОТКАЗ БАЗЫ НАДО ВИДЕТЬ. supabase-js не бросает на отказе — он возвращает
    // его полем error, и один только try/catch ловил бы лишь обрыв сети. Без
    // этой строки отклонённая вставка (ограничение, права, полная база)
    // выглядела бы ровно как успешная: ответ 204, в логе пусто, и узнать, что
    // журнал перестал писать, было бы неоткуда.
    //
    // Пишем в лог, а не человеку: ответ остаётся 204 в любом случае. Счётчик
    // не имеет права ни сломать приложение, ни объясняться с браузером — это
    // наша поломка, и разбираться с ней нам.
    const { error } = await db.from('app_events').insert(rows)
    if (error) console.error(`app_events: не записано ${rows.length} событий:`, error)
  } catch (e) {
    console.error('app_events: запись не удалась:', e)
  }
