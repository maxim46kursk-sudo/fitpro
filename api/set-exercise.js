...
/** Сколько текста уходит в сообщение: его читают с телефона, простыня не нужна. */
const ALERT_TEXT_MAX = 300
...
export default async function handler(req, res) {
  ...
  if (req.query?.action === 'motion-log') {
    ...
    const строки = lines.map(motionLogLine)
    ...
    if (жалоба) тревога('user.report', `🙋 Жалоба из Motion\nсессия ${session}\n${жалоба}`)
    if (insErr) {
      /**
