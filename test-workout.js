...
const mentionsReps = (text) => /\d+\s*(повтор|раз)\b|кг\s*на\s*\d+/i.test(text)
...
mk({
  ...
  extra: (t) => {
    ...
    if (!/(добав\S*[^.?!]{0,30}подход|ещё[^.?!]{0,15}подход|дополнительн\S*[^.?!]{0,15}подход|(третий|четвёртый|пятый)\s+(рабочий\s+)?подход|подход\s+сверху|\d\s*рабочих?\s+подход\S*[^.?!]{0,15}вместо|вместо[^.?!]{0,15}(обычны[хй]|двух)[^.?!]{0,15}подход)/i.test(t)) issues.push('не предложил добавить дополнительный рабочий подход вместо роста веса')
    return issues
  },
})

// Правило 2: база часто тяжело (≥3 оценённых подходов, ≥50% тяжело за 14 дней),
// изоляция без данных (<3 подходов) → не форсировать базу.
const RULE2_HISTORY = [
...
mk({
  group: 'Правила реакции на нагрузку', name: 'правило 2 — часто тяжело на базе, изоляция без данных',
  sets: RULE2_HISTORY, user: 'может добавить вес на приседаниях сегодня?',
  expect: 'Не форсирует рост веса на базовом упражнении, предлагает пока его не трогать',
  extra: (t) => {
    const kgs = kgNumbers(t)
    return kgs.length && Math.max(...kgs) > 34 ? [`похоже согласился увеличить вес на базовом упражнении (${Math.max(...kgs)}кг), хотя должен был придержать`] : []
  },
})
...
mk({
  ...
  extra: (t) => !/(легче|полегче|снизь|снизим|уменьш|отдохн|разгруз|поберег|меньше)/i.test(t) ? ['не предложил снизить нагрузку/разгрузочный режим'] : [],
})
...
const PROGRESS_HISTORY = [
  S('Приседания', 21, 20, 11, 2), S('Приседания', 21, 20, 11, 2), S('Приседания', 21, 24, 11, 2), S('Приседания', 21, 24, 11, 2),
]
mk({
  group: 'Прогрессия', name: 'лёгкие подходы — вес должен вырасти в следующий раз',
  sets: PROGRESS_HISTORY, user: 'сегодня снова та же тренировка что в прошлый раз, что взять по весу на приседаниях',
  expect: 'Называет вес выше прошлого максимума (24кг) и конкретное число повторений — прогрессия, а не топтание на месте',
  extra: (t) => {
    const kgs = kgNumbers(t)
    const issues = []
    if (!kgs.length) issues.push('нет конкретного веса в кг')
    else if (Math.max(...kgs) <= 24) issues.push(`вес не вырос относительно прошлого раза (${Math.max(...kgs)}кг ≤ 24кг)`)
    if (!mentionsReps(t)) issues.push('нет конкретного числа повторений')
    return issues
  },
})
...
mk({
  ...
  extra: (t) => {
    const issues = []
    if (!/(новичок|начина|перв(ый|ая) раз|только начал|с малого|небольш\S* вес|легк\S* вес|простого)/i.test(t)) issues.push('нет дружелюбного комментария про старт с лёгкого для новичка')
    const program = extractSetProgram(t)
