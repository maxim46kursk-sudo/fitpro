...
const ступени = ответ?.час?.воронка?.ступени?.map((с) => с.ступень) ?? []
assertEqual('глубина не влезла в ряд ступеней',
  ступени.filter((ш) => String(ш).startsWith('depth')),
  [])
/**
 * Обе дорожки на месте: чтение (open -> ... -> paid) и игра, добавленная,
 * когда цена ушла с первого экрана (open -> play -> ... -> save).
 */
assertEqual('в ряду обе дорожки — и чтение, и игра', ступени,
  ['open', 'play', 'camera', 'calibrated', 'round-start', 'round-end',
    'up', 'down', 'save', 'scroll', 'join-click', 'auth', 'pay-start', 'paid'])
assertEqual('открывших по-прежнему трое', ответ?.час?.воронка?.ступени?.[0]?.людей, 3)

/**
 * ДОЛЯ СЧИТАЕТСЯ ОТ ТОГО ШАГА, ЗА КОТОРЫМ СТУПЕНЬ ИДЁТ НА САМОМ ДЕЛЕ, а не от
 * строки выше. Дорожки расходятся сразу после `open`, и «от предыдущей» начало
 * бы сравнивать разное: `scroll` вышел бы долей от `save`, то есть дорожка
 * чтения — от хвоста дорожки игры.
 */
const базы = Object.fromEntries((ответ?.час?.воронка?.ступени ?? []).map((с) => [с.ступень, с.от]))
assertEqual('scroll считается от open, а не от save', базы.scroll, 'open')
assertEqual('join-click — от scroll, а не от round-end', базы['join-click'], 'scroll')
assertEqual('up и down — оба от round-end, а не друг от друга',
  [базы.up, базы.down], ['round-end', 'round-end'])
assertEqual('save — от down', базы.save, 'down')
assertEqual('у первой ступени базы нет', базы.open, null)
