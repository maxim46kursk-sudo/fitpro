...
console.log('\n── Кейс 15: parseTemplateSets — дробный вес через запятую ───────────')

{
  // "2,5 кг × 12" — split(',') без нормализации рвал бы это на "2" и
  // "5 кг × 12" (два мусорных подхода). Разделитель между подходами (после
  // запятой пробел, не цифра) при этом не должен пострадать.
  const sets = parseTemplateSets('2,5 кг × 12, 3 кг × 10')
  assertDeepEqual('Кейс 15: "2,5 кг × 12, 3 кг × 10" → два подхода, дробный вес разобран',
    sets, [{ templateKg: 2.5, bandLevel: null, reps: 12 }, { templateKg: 3, bandLevel: null, reps: 10 }])
}
