...
// Есть ли этот коммит на origin. Смысл строчки практический: снимок читают в
// НОВОМ чате, и первое, что там надо понять, — на чём остановились. Ссылка на
// коммит, которого нет в удалённом репозитории, отвечает на этот вопрос
// неверно: продолжать с него нельзя, пока он не запушен.
let pushed = false
let ahead = ''
try {
  ...
  ahead = execSync('git log --oneline origin/main..HEAD').toString().trim().split('\n').filter(Boolean).length
}
...
const lines = [
  ...
  '',
  `Собрано ${stamp} из ${head} (main).`,
  pushed
    ? 'Коммит запушен на origin/main — снимок соответствует тому, что лежит в репозитории.'
    : `ВНИМАНИЕ: коммит НЕ запушен (${ahead} впереди origin/main). Продолжать с него нельзя, пока он не уедет на origin.`,
  ...(dirty ? ['ВНИМАНИЕ: рабочее дерево было не чистым — снимок мог разойтись с коммитом.'] : []),
  '',
