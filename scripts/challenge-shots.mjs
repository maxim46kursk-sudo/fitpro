...
if (has('--preview')) {
  ...
  for (const size of SIZES) {
    ...
    if (герой.нет) {
      ...
      /**
       * Запрещена ЦЕНА БИЛЕТА, а не рубль вообще: призовая сумма на первом
       * экране стоит намеренно (см. playFirst.test.jsx, там же почему).
       */
      const цена = /2\s?990|Участвовать|цена|стоимость/i.exec(герой.текст)
      if (цена) problems.push(`${size.name}: ЦЕНА НА ПЕРВОМ ЭКРАНЕ — «${цена[0]}»`)
      if (!/59\s?970/.test(герой.текст)) problems.push(`${size.name}: призовой суммы на первом экране нет`)
